...
export default function SalesAdmin() {
  ...
  salesStore.allSales.map((sale:any) => {
    data.push({name: sale._id, status: sale.status, count: sale.products.length, total: sale.total, active: sale, details: sale._id})
  });
              ...
              <Tr {...row.getRowProps()} key={Math.random()}>
                {row.cells.map(cell => {
                  if(cell.column.Header === "Ver detalles"){
                    return <Td key={Math.random()}><LightMode><Button><Link to={`/Admin/sales/view/${cell.value}`}><HiOutlinePencilAlt size={20}/></Link></Button></LightMode></Td>
