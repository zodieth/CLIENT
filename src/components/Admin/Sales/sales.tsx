...
export default function SalesAdmin() {
  ...
  const filteredData = data.filter((d:any) =>
    d.name.toLowerCase().includes(searchTerm.toLowerCase()) || d.status.toLowerCase().includes(searchTerm.toLowerCase())
  );
              ...
              <Tr {...row.getRowProps()} key={Math.random()}>
                {row.cells.map(cell => {
                  if(cell.column.Header === "Ver detalles"){
                    return <Td key={Math.random()}><LightMode><Button colorScheme='blue' onClick={() => handleClickOpenModal(cell.value)}><ImEye size={20}/></Button></LightMode></Td>
                  } else if(cell.column.Header === "Enviar"){
                    return <Td key={Math.random()}><LightMode><Button isDisabled={cell.value.status === 'shipped'} colorScheme='blue' onClick={() => handleChangeStatus(cell.value._id)}><ImTruck size={20}/></Button></LightMode></Td>
